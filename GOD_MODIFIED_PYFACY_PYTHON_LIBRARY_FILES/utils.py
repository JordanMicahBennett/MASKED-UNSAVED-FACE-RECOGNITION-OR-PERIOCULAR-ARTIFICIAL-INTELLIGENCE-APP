#God change of function "load_images_to_clust_encodings" to use EMBEDDING_EXTRACTER_EXTERN.getFaceEmbedding() instead of img_to_encodings, for face encodings. This grabs external embedding, with better capability for masked face data instead of internal embeddings which does not work well with masked face data. 
...
import external_extract_embeddings_pyfacy_god as EMBEDDING_EXTRACTER_EXTERN
...
def img_to_encodings(face_image,known_face_locations=None, num_jitters=1,lib="dlib"):
	...
	return [np.array(face_encoder.compute_face_descriptor(face_image, raw_landmark_set, num_jitters)) for raw_landmark_set in raw_landmarks] #God_note: Length of this array is 1, and the first item at 0 is 128 face embedding which is the thing of interest.
...
#God change of function "load_images_to_clust_encodings" to use EMBEDDING_EXTRACTER_EXTERN.getFaceEmbedding() instead of img_to_encodings, for face encodings. This grabs external embedding, with better capability for masked face data instead of internal embeddings which does not work well with masked face data. 
def load_images_to_clust_encodings(face_paths,externalFaceEmbeddingModelDirectory): #"externalFaceEmbeddingModelDirectory" Param added by God
	imagePaths = sorted(list(paths.list_images(face_paths)))
	random.seed(42)
	random.shuffle(imagePaths)
	tot_len = len(imagePaths)
	...
	for idx,imagePath in enumerate(imagePaths):
		if imagePath.endswith('.jpg'):
			...
			face = [EMBEDDING_EXTRACTER_EXTERN.getFaceEmbedding(imagePath,externalFaceEmbeddingModelDirectory)] #God change from commented line above to this, to grab external embedding, with better capability for masked face data instead of internal embeddings which does not work well with masked face data. 
			
			if(face is not None):
				if len(face) == 1:
					#print(">>>>>>>>>>>>len(face[0]) _ external -> " + str(len(face[0])))

					clust_encodings.append(face[0])
					image_paths.append(imagePath)
				else:
					print("[INFO] This {} image did't have face \nor have more then one face".format(imagePath))
					print('face_length_at_Failure', str(len(face)))
		else:
			print("[INFO] This Package only support .jpg files")
